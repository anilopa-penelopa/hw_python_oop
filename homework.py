...
class Running(Training):
    ...
    def get_spent_calories(self) -> float:
        ...
        calories: float = (
            ...
            self.duration * self.MIN_PER_HOUR
        )
        return calories
...
class SportsWalking(Training):
    ...
    def get_spent_calories(self) -> float:
        ...
        calories = (
                 ...
                 * self.coeff_calorie_4 * self.weight) * self.duration * self.MIN_PER_HOUR
        )
        return calories
...
class Swimming(Training):
    ...
    def get_mean_speed(self) -> float:
        ...
        mean_speed_swim = self.length_pool * self.count_pool / self.M_IN_KM / self.duration
        return mean_speed_swim

    def get_spent_calories(self) -> float:
        """Получить количество затраченных калорий на тренеровке "Плавание"."""
        calories = (
            (self.get_mean_speed() + self.coeff_calorie_6) * self.coeff_calorie_7 * self.weight)
        return calories
...
def read_package(workout_type: str, data: list) -> Training:
    ...
    dict_train = {'SWM': Swimming,
                  'RUN': Running,
                  'WLK': SportsWalking}
    train_name = dict_train[workout_type]
    return train_name(*data)
